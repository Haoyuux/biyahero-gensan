...
const target = '      <div className="flex-1 p-6 md:p-8 overflow-y-auto w-full">';
...
const tabReplacement = '              onClick={() => { setActiveTab(id); setIsSidebarOpen(false); }}';
...
if (newContent.includes(target)) {
    ...
    const regex = /^\s*<div className="flex-1 p-6 md:p-8 overflow-y-auto w-full">/m;
    newContent = newContent.replace(regex, (match) => match + replacement.split('\n').slice(1).join('\n'));
}
...
if (newContent.includes(tabTarget)) {
    ...
    newContent = newContent.replace(tabRegex, (match) => match.replace('setActiveTab(id)', '{ setActiveTab(id); setIsSidebarOpen(false); }'));
}
